  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl bg-[#1a1f2e] border-[#2a3142] text-white">
        <DialogHeader>
          <DialogTitle className="text-2xl font-semibold">Create Switch Operator</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <Card className="p-4 bg-[#0f1419] border-[#2a3142]">
            <div className="space-y-3">
              ...
              <div>
                <Label className="text-white">Case Value Parameter</Label>
                <Input
                  value={config.caseValueParam || ''}
                  onChange={(e) => setConfig({ ...config, caseValueParam: e.target.value })}
                  placeholder="e.g., ${workflow.input.status}"
                  className="mt-1 bg-[#1a1f2e] text-white border-[#2a3142]"
                />
              </div>
              <div>
          ...
          <div className="p-3 bg-blue-500/10 border border-blue-500/50 rounded-lg">
            <p className="text-xs text-blue-400">
              Note: Use the JSON editor to define decision cases and default case.
            </p>
          </div>
