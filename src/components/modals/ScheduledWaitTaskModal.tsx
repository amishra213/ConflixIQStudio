...
interface ScheduledWaitTaskConfig {
  orgId: string;
  taskRefId: string;
  taskId: string;
  taskType: string;
...
interface ScheduledWaitTaskModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSave: (config: ScheduledWaitTaskConfig) => void;
  initialConfig?: ScheduledWaitTaskConfig | null;
}
...
export function ScheduledWaitTaskModal({
  open,
  onOpenChange,
  onSave,
  initialConfig,
}: ScheduledWaitTaskModalProps) {
  const [config, setConfig] = useState<ScheduledWaitTaskConfig>({
    orgId: 'ORG001',
    taskRefId: '',
    taskId: '',
    taskType: 'SCHEDULED_WAIT', // Renamed task type
  ...
  useEffect(() => {
    if (open && initialConfig) {
      ...
      const params = Object.entries(initialConfig.taskinputParameters).map(([name, value], index) => ({
        id: `${Date.now()}-${index}`,
        name,
        value
      }));
      
      setInputParameters(params.length > 0 ? params : [
        { id: '1', name: 'pauseDuration', value: '${workflow.input.pauseDuration}' },
        { id: '2', name: 'pauseDurationUnits', value: '${workflow.input.pauseDurationUnits}' }
      ]);
      ...
      setExecutionRefs(execRefs.length > 0 ? execRefs : [
        { id: '1', name: 'genericTaskName', value: 'codeExecuter_generic_task' }
      ]);
    } else if (open) {
      ...
      const defaultConfig = {
        orgId: 'ORG001',
        taskRefId: `scheduledwait-${timestamp}`,
        taskId: `scheduled-wait-${timestamp}`,
        taskType: 'SCHEDULED_WAIT', // Renamed task type
        taskListDomain: 'BOPUS-TASKLIST',
        sequenceNo: 1,
        executionNamespace: 'ORDER',
  ...
  const handleSave = () => {
    ...
    const finalConfig: ScheduledWaitTaskConfig = {
      orgId: config.orgId,
        ...
        <div className="flex-1 overflow-y-auto px-6">
          <Tabs defaultValue="basic" className="space-y-4 py-4">
            ...
            <TabsContent value="basic" className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., scheduledwait-1760532809257"
                      className="mt-2 bg-[#1a1f2e] text-white border-[#2a3142]"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="e.g., scheduled-wait-1"
                      className="mt-2 bg-[#1a1f2e] text-white border-[#2a3142]"
                    />
                  </div>
                  <div>
              ...
              <Card className="p-6 bg-[#0f1419] border-[#2a3142]">
                <h3 className="text-lg font-semibold text-white mb-3">About Scheduled Wait</h3>
                <div className="space-y-3 text-sm text-gray-300">
                  <p>
                    This task type allows workflows to pause execution for a specified duration.
                  </p>
                  ...
                  <p className="text-xs text-gray-400 mt-3">
                    Common use cases: Remorse periods, scheduled delays
                  </p>
                </div>
              </Card>
            </TabsContent>
