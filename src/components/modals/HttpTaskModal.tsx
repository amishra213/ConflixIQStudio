...
export function HttpTaskModal({
  ...
  useEffect(() => {
    if (open) {
      const defaultConfig: HttpTaskConfig = {
        ...
        taskinputParameters: {},
        // input and output are not part of defaultConfig here
      };

      if (initialConfig) {
        const mergedConfig: HttpTaskConfig = {
          ...defaultConfig,
          ...initialConfig,
          retryPolicy: {
            ...defaultConfig.retryPolicy,
            ...(initialConfig.retryPolicy || {}),
          },
          timeoutPolicy: {
            ...defaultConfig.timeoutPolicy,
            ...(initialConfig.timeoutPolicy || {}),
          },
          httpRequest: {
            ...defaultConfig.httpRequest,
            ...(initialConfig.httpRequest || {}),
          },
          input: initialConfig.input || {}, // Ensure input is merged
          output: initialConfig.output || {}, // Ensure output is merged
        };
        setConfig(mergedConfig);
        setHeadersJson(JSON.stringify(initialConfig.httpRequest?.headers || {}, null, 2));
        setBodyJson(JSON.stringify(initialConfig.httpRequest?.body || {}, null, 2));
        setInputJsonState(JSON.stringify(initialConfig.input || {}, null, 2));
        setOutputJsonState(JSON.stringify(initialConfig.output || {}, null, 2));
  ...
  const handleSave = () => {
    ...
    if (!validateJson(headersJson) || !validateJson(bodyJson)) {
      return;
    }
                    ...
                    <Textarea
                      ...
                      onChange={(e) => {
                        ...
                        try {
                          ...
                          // If invalid, set to empty object to avoid breaking JSON.stringify in preview
