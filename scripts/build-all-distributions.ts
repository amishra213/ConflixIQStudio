...
function setupLocalTempDir(): string {
  const localTempDir = path.join(ROOT_DIR, '.build-temp');
  if (!fs.existsSync(localTempDir)) {
    fs.mkdirSync(localTempDir, { recursive: true });
    log(`Created local temp directory: ${localTempDir}`);
  }
  return localTempDir;
}

function getBuildEnvironment(localTempDir: string): NodeJS.ProcessEnv {
  return {
    ...process.env,
    TEMP: localTempDir,
    TMP: localTempDir,
    USERPROFILE: localTempDir,
...
function copyWindowsExeFiles(distPath: string, windowsDir: string): string[] {
  const files: string[] = [];
  if (fs.existsSync(distPath)) {
...
function cleanupTempDir(localTempDir: string): void {
  if (fs.existsSync(localTempDir)) {
    try {
      fs.rmSync(localTempDir, { recursive: true, force: true });
      log(`Cleaned up local temp directory`);
    } catch (cleanupError) {
      log(`⚠ Warning: Could not clean up temp directory: ${cleanupError}`);
    }
  }
}
...
function buildWindows(): string[] {
  ...
  try {
    const localTempDir = setupLocalTempDir();
    const buildEnv = getBuildEnvironment(localTempDir);

    log('Building with local temp directory to avoid AppData path issues...');
    ...
    const files = copyWindowsExeFiles(distPath, windowsDir);
    cleanupTempDir(localTempDir);
    
    return files;
  } catch (e) {
    const errorMsg = e instanceof Error ? e.message : String(e);
    if (errorMsg.includes('symbolic link') || errorMsg.includes('privilege')) {
      log(`⚠ Windows build skipped: Code signing infrastructure requires elevated privileges`);
      log(`  (This is a known issue on Windows without admin. Windows builds can be created on CI.)`);
    } else {
      error(`Failed to build Windows: ${errorMsg}`);
    }
    return [];
  }
}
...
function buildDocker(): string[] {
  ...
  try {
    ...
    try {
      // Use PowerShell or native tools depending on platform
      if (process.platform === 'win32') {
        // On Windows, use 7zip or native compression if available
        // For now, we'll skip TAR.GZ and keep TAR (which is already compressed by Docker)
        log('TAR.GZ compression skipped on Windows (TAR file is already available)');
      } else {
