...
export function Tasks() {
  ...
  const handleTaskTypeSelect = (taskType: string) => {
    ...
    switch (taskType) {
      case 'GENERIC': setIsGenericModalOpen(true); break;
      case 'HTTP': setIsHttpModalOpen(true); break;
      case 'MAPPER': setIsMapperModalOpen(true); break;
      case 'WAIT_FOR_SIGNAL': setIsWaitForSignalModalOpen(true); break;
    }
  };
  ...
  const handleSaveWaitForSignalTask = (config: any) => {
    addTask({ id: `task-${Date.now()}`, name: config.taskId, type: 'WAIT_FOR_SIGNAL', description: 'Wait for signal worker task', config });
  ...
  return (
    <div className="p-8 space-y-8 bg-[#0f1419]">
      ...
      <GenericTaskConfigModal open={isGenericModalOpen} onOpenChange={setIsGenericModalOpen} node={null} onSave={handleSaveGenericTask} />
      <HttpTaskModal open={isHttpModalOpen} onOpenChange={setIsHttpModalOpen} onSave={handleSaveHttpTask} initialConfig={null} />
      <MapperTaskModal open={isMapperModalOpen} onOpenChange={setIsMapperModalOpen} onSave={handleSaveMapperTask} initialConfig={null} />
      <WaitForSignalTaskModal open={isWaitForSignalModalOpen} onOpenChange={setIsWaitForSignalModalOpen} onSave={handleSaveWaitForSignalTask} initialConfig={null} />
    </div>
  );
}
